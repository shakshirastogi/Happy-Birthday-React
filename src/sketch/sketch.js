...
export default (parent, text) => (sketch) => {
  ...
  const fillDots = (width, height) => {
    dots = [];
    let [firstWord, secondWord, thirdWord ,forthword] = text.split(' ') // Assumption that text is just 3 words, no more, no less
    ...
    if (width > height) {
      ...
      secondWordPoints = font.textToPoints(`${thirdWord} ${forthword}`,width * 0.05, height * 0.83, width * 0.12);
    } else {
